...
eia_data_weekly_updated <- eia_updated_tab_weekly[, setnames(.SD, 2:3, c("name", as.character(d)))
  ][, .SD, .SDcols = 2:3
...
eia_data_weekly <- eia_data_weekly[, week := 1:.N, keyby = .(name, year)
  ][,tag := week[year == max(year)] %>% max(), by = .(name) # 有些week在最新年份还没到，需要加一个tag
  ][, ':='(
...
eia_updated_pic_weekly <- list(eia_data_weekly[,
  ...
  ][, unique(.SD)], eia_data_weekly) %>% rbindlist(use.names = T, fill = T)
...
# 3. 把表和图都拼起来
eia <- tab + pic
ggsave("./pic/eia.jpg", plot = eia, device = "jpg", dpi = 150, width = 90, height = 40, limitsize = F)
