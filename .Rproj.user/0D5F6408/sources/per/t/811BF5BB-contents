...
# 1. Retrive 2017-2022 data: table9.csv and save the list data
eia_web <- data.table(url_part =  html_nodes(urlpage, xpath = '//div[@class="main_col"]//a') |> html_attr("href"))
eia_web <- eia_web[!is.na(url_part), .SD[-1]
    ][, url_part := str_replace(url_part, "/wpsr(.+)php", "")
    ][, url_part := str_c("https://www.eia.gov", url_part, "/csv/table9.csv")
    ][str_detect(url_part, paste((year(Sys.Date())-5):year(Sys.Date()), collapse = "|")), .SD
    ][, .(data = fread_sleep(url_part, encoding = "UTF-8") %>% list()), by = .(url_part)]

save(eia_web, file = "./data/eia_web_history.Rdata")
...
eia_weekly_clean <- function(x) {
  x[!str_detect(STUB_2, "PADD")
      ...
      ][, date := as.character(date) %>% as.Date("%m/%d/%y")
      ][, ':='(year = year(date), month = month(date))
      ][]
}

eia_data_weekly <- eia_web[, lapply(data, eia_weekly_clean) %>% rbindlist(), by = .(url_part)
    ][, url_part := NULL
